...
const triggerConfetti = () => {
  ...
  const interval: any = setInterval(() => {
    ...
    confetti({ ...defaults, particleCount, origin: { x: randomInRange(0.1, 0.3), y: Math.random() - 0.2 }, colors: ['#0057a6', '#ffd700', '#ff4040', '#00cc66'] });
    confetti({ ...defaults, particleCount, origin: { x: randomInRange(0.7, 0.9), y: Math.random() - 0.2 }, colors: ['#0057a6', '#ffd700', '#ff4040', '#00cc66'] });
  }, 250);
};

const DayProgressCard = () => {
  const [discount, setDiscount] = useState<number>(0);
  const [displayDiscount, setDisplayDiscount] = useState<string | number>(0);
  ...
  return (
    ...
    >
      <button onClick={() => setVisible(false)} className="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
        <X size={18} />
      </button>

      {/* Animated particles */}
      <div className="relative">
        <motion.div className="absolute -top-10 -left-8 w-8 h-8 bg-blue-400 rounded-full"
          animate={{ y: [0, -10, 0], opacity: [0.7, 1, 0.7] }}
          transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div className="absolute -top-4 -right-6 w-6 h-6 bg-yellow-400 rounded-full"
          animate={{ y: [0, -8, 0], opacity: [0.5, 1, 0.5] }}
          transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut", delay: 0.3 }}
        />
        <motion.div className="absolute -bottom-8 left-6 w-10 h-10 bg-green-400 rounded-full"
          animate={{ y: [0, -12, 0], opacity: [0.6, 1, 0.6] }}
          transition={{ duration: 4, repeat: Infinity, ease: "easeInOut", delay: 0.7 }}
        />
        <motion.div className="absolute -bottom-2 right-4 w-5 h-5 bg-red-400 rounded-full"
      ...
      <div className="absolute -top-5 -left-5 w-20 h-20 bg-red-500 rounded-full flex items-center justify-center transform rotate-[-15deg] shadow-lg">
        <div className="text-white font-bold text-center leading-tight">
          ...
          <div className="text-xl">{displayDiscount}</div>
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        <div className="flex justify-center mb-4 mt-6">
          <div className="relative w-32 h-32 flex flex-col items-center justify-center bg-yellow-50 rounded-full border-4 border-dashed border-yellow-500">
            ...
            <div className="text-4xl font-bold text-red-600">{displayDiscount}</div>
